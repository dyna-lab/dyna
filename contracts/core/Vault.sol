...
contract Vault is IVault, Ownable, BlastBase {
    ...
    function buyUSD(address _token) external override onlyManager returns (uint256) {
        _validate(tokenBase[_token].isFundable, 16);
    ...
    function increasePosition(address _account, address _collateralToken, address _indexToken, uint256 _sizeDelta, bool _isLong) external override {
        ...
        require(tokenBase[_collateralToken].isFundable, "invalid colToken");
        require(tokenBase[_indexToken].isTradable, "invalid index Token");
        ...
        _validate(position.sizeUSD > 0, 30);
        _validatePosition(position.sizeUSD, position.collateralUSD);

        require(position.collateralUSD.mul(IVaultUtils(vaultUtils).maxLeverage()) > position.sizeUSD, "maxLev");
        require(position.collateralUSD < position.sizeUSD, "max collateral");
        // vaultUtils.validateLiquidationPos(position, true);//TODO: opt.to save gas

        // reserve tokens to pay profits on the position
        {
            uint256 _newReserveAmount = usdToTokenMax(_collateralToken, position.sizeUSD.add(position.collateralUSD));
            ...
            else if (position.reserveAmount < _newReserveAmount){
    ...
    function _decreasePosition(
...

        {
            ...
            if (unPft >= position.collateralUSD){
                ...
                require(position.collateralUSD > unPft, "insufficient collateral");
                require((position.collateralUSD - unPft) * vaultUtils.maxLeverage() > position.sizeUSD, "max leverage");
        ...
        if (position.collateralUSD < 1 || posIsLiq ){
            ...
            if (_collateralDeltaUsd > 0){
                require(position.collateralUSD >= _collateralDeltaUsd, "invalid colDelta");
            ...
            uint256 _newReserveAmount = usdToTokenMax(position.collateralToken, position.sizeUSD.add(position.collateralUSD));
    ...
    function liquidatePosition(address _account, address _collateralToken, address _indexToken, bool _isLong, address _feeReceiver) external override {
        ...
        require(posIsLiq, "Not Liq");
    }
    
    //---------- PUBLIC FUNCTIONS ----------
    function directPoolDeposit(address _token) external override {
        _validate(tokenBase[_token].isFundable, 14);
        uint256 tokenAmount = _transferIn(_token);
    ...
    function _swap(address _tokenIn,  address _tokenOut, address _receiver ) private returns (uint256) {
        _validate(tokenBase[_tokenIn].isSwappable, 24);
        _validate(tokenBase[_tokenOut].isSwappable, 25);
    ...
    function _validatePosition(uint256 _size, uint256 _collateral) private view {
        ...
        require(_size < _collateral * vaultUtils.maxLeverage(), "max leverage");
